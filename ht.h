...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if( /* Fill me in */this->numProbes_ == this->m_ ) {
            return this->npos; 
        }
...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
    for (size_t i = 0; i < table_.size(); i++) {
        if (table_.at(i) && !table_.at(i)->deleted) {
            return false;
        }
    }
    return true;
...
{
    if (real_size >= CAPACITIES[mIndex_] * alpha) {
        resize();
    }
    
    HASH_INDEX_T idx = probe(p.first);
    if (idx == npos) {
        throw std::logic_error("Full table");
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(/* Fill me in */!table_[loc]->deleted && kequal_(key, table_[loc]->item.first)) {
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
...
void HashTable<K, V, Prober, Hash, KEqual>::reportAll(std::ostream& out) const
{
	for(HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_]; ++i)
	{
		if(table_[i] != nullptr)
		{
			out << "Bucket " << i << ": " << table_[i]->item.first << " " << table_[i]->item.second << " "  << (table_[i]->deleted?1:0) << std::endl;
		}
	}
}
